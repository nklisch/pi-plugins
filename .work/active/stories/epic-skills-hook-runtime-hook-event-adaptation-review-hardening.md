...
- [ ] `pi.toolResult.content` shares no mutable array/object identity with Pi event evidence.
- [ ] Post-plan source mutation leaves the plan byte-for-byte unchanged.
- [ ] Optional `textSignature` is safely omitted and does not reject a normal tool result.
- [ ] Existing tool success/failure/interrupt and exact input goldens remain unchanged.
