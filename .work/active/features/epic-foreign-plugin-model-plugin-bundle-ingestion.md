...
export type ComponentLogicalIdentity =
  ...
  | Readonly<{ kind: "foreign"; nativeHost: NativeHost; nativeKind: string; declarationSubkey: string }>;
...
`createForeignComponentDeclaration` validates raw JSON, native host/kind, a semantic declaration subkey (default role or stable keyed/list member), and provenance, then the bundle merger derives its id. Pointer spelling is provenance only. Foreign components receive no diagnostic solely for being unsupported: they are valid complete inventory for the reporting feature.
