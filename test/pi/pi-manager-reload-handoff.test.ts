...
describe("Pi manager reload handoff", () => {
  it("moves only a schema-valid plain envelope to the exact successor", async () => {
    ...
    expect(handoff.publish(ticket, envelope)).toBe("successor");
    await expect(claim?.result).resolves.toEqual(envelope);
    expect(() => handoff.publish(ticket, envelope)).toThrow(/settled|unknown/);
  });
  ...
  it("returns local ownership when no reload successor claimed", () => {
    ...
    expect(handoff.publish(ticket, envelope)).toBe("local");
  ...
  it("rejects duplicate session slots and malformed envelopes", () => {
    ...
    expect(() => handoff.publish(ticket, { ...envelope, data: { secret: Symbol("x") } } as never)).toThrow();
