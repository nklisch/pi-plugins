...
describe("native control execution admission", () => {
  it("isolates concurrent IDs/counters and preserves ordered results", async () => {
    ...
    const dispatch = async (_command: any, context: any) => { await context.progress.emit({ phase: "preflight", state: "started" }); return { status: "ok", data: {}, diagnostics: [], human: [] }; };
  ...
  it("keeps a committed result stronger than output failure", async () => {
    ...
    const report = await coordinator.execute(command(), { sink }, async (_command, context) => {
      ...
      return { status: "ok", data: { committed: true }, diagnostics: [], human: [] };
    }, new AbortController().signal);
    expect(report.envelope).toMatchObject({ status: "ok", data: { committed: true } });
  ...
  it("propagates injected timeout cancellation before semantic evidence", async () => {
    ...
    const report = await coordinator.execute(command(), { timeoutMs: 10 }, async (_command, _context, signal) => { signal.throwIfAborted(); return { status: "ok", data: {}, diagnostics: [], human: [] }; }, new AbortController().signal);
  ...
  it("quiesces new work and drains admitted work idempotently", async () => {
    ...
    const running = coordinator.execute(command(), {}, async () => { await gate; return { status: "ok", data: {}, diagnostics: [], human: [] }; }, new AbortController().signal);
    coordinator.quiesce();
