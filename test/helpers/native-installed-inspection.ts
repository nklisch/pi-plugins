...
export function createNativeInstalledHarness(options: NativeInstalledHarnessOptions = {}) {
  ...
  const snapshot = {
    ...
    ...(options.updatePending ? { hostStatus: { status: "ready", local: { recovery: "settled", runtime: "reconciled" }, update: { state: "running", unreadCount: 1, unresolvedCount: 1 }, blocked: [], capabilities: { mcp: { status: "available", explanation: "ready" }, subagents: { status: "unavailable", explanation: "none" }, piReload: { status: "available", explanation: "ready" }, secrets: { status: "available", explanation: "ready" } } } } : {}),
  } as any;
