...
describe("packaged marketplace discovery in a clean environment", () => {
  it("adds, lists, searches, and details a local marketplace without foreign CLIs", async () => {
    ...
    try {
      ...
      const listed = await started.application.marketplace.registration.list({ scope: "all-current", limit: 50 }, new AbortController().signal);
      expect(listed.registrations).toHaveLength(1);
      const page = await started.application.marketplace.catalog.search({ scope: "all-current", query: "offline demo", limit: 50 }, new AbortController().signal);
      ...
      await expect(started.application.marketplace.catalog.detail({ candidateId: candidate.id, snapshot: candidate.snapshot }, new AbortController().signal))
