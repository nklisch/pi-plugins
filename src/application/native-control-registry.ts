...
const MarketplaceAddControlSchema = z.object({ source: MarketplaceSourceSchema, scope: ScopeSchema }).strict().readonly();
const MarketplaceRemoveControlSchema = z.object({ registrationId: MarketplaceRegistrationIdSchema, scope: ScopeSchema, confirmed: z.literal(true) }).strict().readonly();
const MarketplaceListControlSchema = z.object({ scope: ReadScopeSchema.default("all-current"), limit: LimitSchema.default(50) }).strict().readonly();
const MarketplaceRefreshControlSchema = z.object({ scope: ReadScopeSchema.default("all-current"), registrationIds: z.array(MarketplaceRegistrationIdSchema).readonly().optional() }).strict().readonly();
const AdoptionPreviewControlSchema = z.object({ scope: ReadScopeSchema.default("all-current") }).strict().readonly();
const AdoptionImportControlSchema = z.object({ candidateIds: z.array(AdoptionCandidateIdSchema).min(1).readonly(), scope: ScopeSchema, confirmed: z.literal(true) }).strict().readonly();
